...
function extractProductFromCard(card) {
  ...
  const description =
    card.querySelector("p")?.textContent.trim() || defaultProduct.description;
  const price =
    card.querySelector(".price")?.textContent.trim() || defaultProduct.price;
  const tag =
    card.querySelector(".pill")?.textContent.trim() || defaultProduct.tag;
...
function buildWhatsAppUrl(product, details = {}) {
  ...
  if (details.shippingCharge)
    lines.push(`Shipping: ₹${details.shippingCharge}`);
  if (details.totalAmount) lines.push(`Total amount: ₹${details.totalAmount}`);
  if (details.transactionId)
    lines.push(`Transaction ID: ${details.transactionId}`);
...
function attachEventHandlers() {
  ...
  const storedProduct = JSON.parse(
    localStorage.getItem("selectedProduct") || JSON.stringify(defaultProduct),
  );
  ...
  if (page === "product.html" || page === "product") {
    ...
    if (image) {
      ...
      image.onerror = () => {
        image.src = defaultProduct.image;
      };
    }
  ...
  if (page === "checkout.html" || page === "checkout") {
    ...
    document.getElementById("checkoutShipping").textContent =
      formatIndianPrice(shippingCharge);
    ...
    if (checkoutImg) {
      ...
      checkoutImg.onerror = () => {
        checkoutImg.src = defaultProduct.image;
      };
    }
      ...
      ?.addEventListener("submit", (event) => {
        ...
        const pincode =
          formElement
            ...
            ?.value.trim() || "";
        const phone =
          formElement.querySelector('input[type="tel"]')?.value.trim() || "";
        const size = document.getElementById("sizeSelect")?.value || "M";
  ...
  if (page === "payment.html" || page === "payment") {
    ...
    const totalAmount =
      checkoutOrder?.totalAmount || localStorage.getItem("checkoutTotal") || "";
    ...
    document.getElementById("paymentShipping").textContent =
      formatIndianPrice(shippingCharge);
    document.getElementById("paymentTotal").textContent = formatIndianPrice(
    ...
    if (payImg) {
      ...
      payImg.onerror = () => {
        payImg.src = defaultProduct.image;
      };
    }
